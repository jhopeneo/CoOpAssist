...
def render_header():
    ...
    # Show ingestion status banner (non-blocking check)
    try:
        ...
        if _ingestion_status.get("running", False):
            ...
            st.info(
                f"🔄 **Ingestion running** ({elapsed_str}) - Check Documents page for details",
                icon="ℹ️"
            )
    except Exception:
        pass  # Silently fail if status check has issues
